...
// restaurant's fallback number. Values mirror TCD/restaurant.js exactly.
...
const _safeLsKey = (k => {
    ...
    return (_RESTAURANT.prefix || 'tcd') + '_' + k;
});
...
async function collect_data(){
    ...
    let data = {
        ...
        'tcd_place':localStorage.getItem(_safeLsKey('place')) || '',
        'tcd_place_custom': localStorage.getItem(_safeLsKey('place_custom')) === '1',
        'status':'In Progress',
    ...
    try {
        ...
        if (mirror && mirror.mirrorToBankiBites) {
            await mirror.mirrorToBankiBites({
                restaurant_id:   'HOTEL-AMAN-SWEETS',
                ...
                place_custom:    data.tcd_place_custom === true,
