...
const fetchBestsellers = async () => {
    try {
        const res = await fetch(`admin/unique_order_data.json?v=${Date.now()}`);
        if (!res.ok) return;
...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    const buildDishCard = (dish, subcategory) => {
        ...
        if (dish.is_offer && !hasOffer) {
            console.warn(`[Unique] Dish "${dish.name}" is_offer=true but offer_price is missing or ≤ price; skipping strikethrough.`);
        }
        const pct = hasOffer
    ...
    // localStorage['<prefix>_order_history'] — matching the invoice page's
    ...
    // orders (tiebreak by total quantity) so most-frequent picks come first.
