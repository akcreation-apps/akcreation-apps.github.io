...
function initializeFirebase() {
    return get_credentials().then(credentials => {
        ...
        table_name = decrypt_values(credentials.ORDER_TABLE_NAME, _cfg)
        admin_id = _cfg
...
async function fetchOrderDetails(orderId, allOrders) {
    ...
    if (docSnap.exists()) {
        ...
        if (orderData.status === 'Approved') {
            ...
            localStorage.setItem("a1_order_history", JSON.stringify(updatedOrders));
...
async function fetchOrders() {
    const orders = JSON.parse(localStorage.getItem('a1_order_history')) || [];
    ...
    if(orders && orders.length > 0){
        ...
        for (const order of orders) {
            if(!order.api_call || order.api_call === "Initiated"){
                const allOrders = JSON.parse(localStorage.getItem('a1_order_history')) || [];
...
function handleDelete(orderId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let orders = JSON.parse(localStorage.getItem('a1_order_history')) || [];
            ...
            localStorage.setItem('a1_order_history', JSON.stringify(orders));
