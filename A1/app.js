...
const fetchBestsellers = async () => {
    try {
        const res = await fetch(`admin/a1_order_data.json?v=${Date.now()}`);
        if (!res.ok) return;
...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    let disable_ids = localStorage.getItem('a1_disable_item_ids');
    if (disable_ids === null) {
        disable_ids = [];
    } else {
    ...
    onlyVegCheckbox.checked = localStorage.getItem('a1_onlyVeg') === 'true';
    onlyNonVegCheckbox.checked = localStorage.getItem('a1_onlyNonVeg') === 'true';
    ...
    const saveCheckboxState = () => {
        localStorage.setItem('a1_onlyVeg', onlyVegCheckbox.checked);
        localStorage.setItem('a1_onlyNonVeg', onlyNonVegCheckbox.checked);
            ...
            .then(querySnapshot => {
                querySnapshot.forEach(doc => {
                    if (doc.data().whatsapp_no !== undefined) {
                        localStorage.setItem('a1_whatsapp_no', doc.data().whatsapp_no);
                    }
                    ...
                    if (doc.data().shop_status !== undefined) {
                        localStorage.setItem('a1_shop_status', doc.data().shop_status);
                    }
                    if (doc.data().opening_time !== undefined) {
                        localStorage.setItem('a1_opening_time', doc.data().opening_time);
                    }
                    if (doc.data().closing_time !== undefined) {
                        localStorage.setItem('a1_closing_time', doc.data().closing_time);
                    }
                });
               
                // Remove disabled dishes from the cart
                const disableIdsRaw = localStorage.getItem('a1_disable_item_ids');
